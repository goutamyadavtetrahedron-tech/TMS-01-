...
export default function TrainingPage({ params }) {
        ...
        return (
            <div key={key} className={`${colClass} flex flex-col`}>
                ...
                >
                    ...
                    {isNonEmptyString(cardTitle) && <h5 style={dynamicStyles.cardTitle} className="text-lg font-bold text-slate-800 mb-2">{cardTitle}</h5>}
                    {cardDescriptionContent && <div style={dynamicStyles.cardDescription} className="text-slate-600 text-sm leading-relaxed">{cardDescriptionContent}</div>}
                </div>
            </div>
        );
    };
    ...
    const renderPillars = () => {
        ...
        const renderedCards = cardPillars.map((pillar, index) => renderCard(pillar, index, 'pillar', cardPillars.length)).filter(Boolean); // Filter out null results from renderCard
    ...
    const renderKeyConcepts = (dataKey, defaultTitle) => {
        ...
        const renderedCards = isNonEmptyArray(itemsToRender)
            ? itemsToRender.map((item, index) => renderCard(item, index, dataKey, itemsToRender.length)).filter(Boolean) // Filter out null cards
            : [];
    ...
    const renderCourseContent = () => {
        ...
        return (
            <section style={dynamicStyles.dynamicSection}>
                ...
                {introContent && <div className="fs-5 text-center mx-auto mb-4" style={{ maxWidth: "800px" }}>{introContent}</div>}
                {hasImage && (
                    <div className="w-full flex justify-center items-center my-6 text-center">
                        <img
                ...
                <div className="container">
                    {hasLogosArray ? (
                        ...
                        <img
                            ...
                            alt="Client Logos"
                            className="img-fluid"
                            style={{ maxHeight: '1000px', width: 'auto', margin: 'auto' }}
                        />
                        <img
