/**
 * Helper utility to determine dynamic email subjects, dynamic sender names,
 * formatted IST dates, and rich lead notification emails.
 */

export function formatISTDate(inputTime) {
  const d = inputTime ? new Date(inputTime) : new Date();
  ...
  const formattedDate = validDate.toLocaleDateString('en-GB', optionsDate); // e.g., "12 Aug 2026"
  const formattedTime = validDate.toLocaleTimeString('en-US', optionsTime); // e.g., "11:41:23 AM"

  return `${formattedDate}, ${formattedTime} (IST)`;
}

export function getDynamicSenderName(data = {}) {
  const { role, pagePath = '', formType = 'contact' } = data;

  if (formType === 'apply' || role || pagePath.includes('/career')) {
    return 'Tetrahedron Careers';
  }
...
export function formatAdminEmailHtml(data = {}) {
  ...
  const categorySender = getDynamicSenderName(data);
  const heading = isCareer ? `New Job Application: ${role}` : `New Lead / ${categorySender} Inquiry`;
  const timeFormatted = formatISTDate(submissionTime);
...
export function formatAdminEmailText(data = {}) {
  ...
  return `
==================================================
${isCareer ? `NEW JOB APPLICATION: ${role}` : `NEW LEAD / ${categorySender.toUpperCase()} INQUIRY`}
==================================================
